...
inline void print_one_dim_mat(ncnn::Mat mat)
{
    float* ptr = mat.channel(0);
    for (int i = 0; i < 10; i++)
    {
        printf("%f ", *ptr);
        ptr++;
...
inline void print_infer_time_each_photo(double time, const char* path, bool end)
{
    FILE* f = fopen(path, "a");
...
inline void just_print(ncnn::Mat in)
{
    int h, w, d, c;
    h = in.h;
    w = in.w;
...
inline void print_float_mat(ncnn::Mat in, const char* path)
{
    FILE* f = fopen(path, "w");
...
inline ncnn::Mat cstr2mat(const char* cstr) // parse c string of format "[d,c,h,w]" and return a ncnn::mat of the shape
{
    ...
    ncnn::Mat m(w, h, d, c, 4u);
    return m;
}

inline ncnn::Mat cstr2mat(const char* cstr,ncnn::Option opt) // parse c string of format "[d,c,h,w]" and return a ncnn::mat of the shape
{
    int w, h, c, d;
    sscanf(cstr, "[%d,%d,%d,%d]", &d, &c, &h, &w);
    ...
    {
        ncnn::Mat m(w, h, d, c, 4u, opt.blob_allocator);
        return m;
    }
    else {
        ncnn::Mat m(w, h, d, c, 4u, opt.persistence_weight_allocator);
        return m;
    }
}
