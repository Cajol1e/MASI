...
public:
    ...
    {
        total_budgets.resize(x0, std::map<int, int>({{0, y0}})); // initialize budget 0 ~ y
        total_payouts.resize(x0, std::map<int, int>());
    }
    ...
    std::pair<int, int> find_budget_yrange(int x, int y1, int y2, int dy, int opt = FIND_FIRST)
    {
        ...
        // find a budget
        if (opt == FIND_FIRST || opt == FIND_SMALLEST_Y)
        {
            auto it = budget.begin();
            for (; it != budget.end(); ++it)
            {
        ...
        {
            auto it = budget.rbegin();
            std::pair<int, int> ret = {-1, -1};
            for (; it != budget.rend(); ++it)
            {
                if (it->second >= dy && it->first + dy <= y2 && y1 + dy <= it->first + it->second)
                {
                    return *it;
    ...
    int insert_xy(int x, int y, int dy)
    {
        ...
        {   
            ...
            if ((it->first <= y) && (it->first + it->second >= y + dy))
            {
                ...
                if (first < y)
                {   
                    if (budget.count(first) != 0)
                    {
                        budget.find(first)->second = y - first;
                        erase_it = false;
                    }
                    else {
                        budget.insert({first, y - first});
                    }
                }
                if (first + second > y + dy)
                {
                    budget.insert({y + dy, first + second - y - dy});
                }
                if (erase_it)
                    budget.erase(it);
    ...
    int insert_xy(int x, int y, int dy,std::string name)
    {
        if (!is_aligned(y))
        {
            return -2;
        ...
        for (; it != budget.end(); ++it)
        {
            ...
            if ((it->first <= y) && (it->first + it->second >= y + dy))
            {
                ...
                if (name.find("intermediate") != std::string::npos) {
                    if (name_counter.count(name) == 0)
                    {
                        name_counter.insert({name, 1});
                        name_mapper.insert({{x, y}, name});
                    }
                    else
                    {
                        name_counter[name]++;
                        name_mapper.insert({{x, y}, name + "_" + std::to_string(name_counter[name])});
                    }
                }
                else {
                    name_mapper.insert({{x, y}, name});
    ...
    int insert_xrange_y(int x1, int x2, int y, int dy, std::string name)
    {
        if (!is_aligned(y))
        {
            return -10;
    ...
    std::pair<int, int> insert_xrange_yrange(int x1, int x2, int y1, int y2, int dy,std::string name)
    {
        dy = align_big(dy);
        y1 = align_big(y1);
        y2 = align_small(y2);
        ...
        return {start_x, insert_xrange_y(start_x, x2, y, dy, name)};
    }
    ...
    std::pair<int, int> insert_xrange(int x1, int x2, int dy,std::string name)
    {
        dy = align_big(dy);
        // fprintf(stderr, "insert x range: %d %d %d.\n", x1, x2, dy);
        return insert_xrange_yrange(x1, x2, 0, y0, dy, name);
    }

public:
    std::vector<std::map<int, int>> total_budgets;                // total_budgets[x]={(y,dy), ...}.
    std::vector<std::map<int, int> > total_payouts;               // total_payouts[x]={(y,dy), ...}.
    std::vector<std::map<int, int> > backup_budgets;              // backup_budgets[x]={(y,dy), ...}.
    std::vector<std::map<int, int> > backup_payouts;              // backup_payouts[x]={(y,dy), ...}.
    std::map<std::pair<int, int>, std::string> name_mapper;
    std::map<std::pair<int, int>, std::string> backup_name_mapper;
    std::map<std::string, int> name_counter;
    std::map<std::string, int> backup_name_counter;
    int y0;
    int x0;
...
public:
    ...
    {
        printf("start saving payouts data\n");
        ...
        for (int x = 0; x < (end_x > 0 ? end_x : x0); ++x)
        {
            ...
            {   
                ...
                if (name_mapper.count(std::make_pair(x, payout.first)) > 0) {
                    it = name_mapper.find(std::make_pair(x, payout.first));
                    fprintf(fp, "%d,%d,%d,%s\n", x, payout.first, payout.second, it->second.c_str());
                }
                else {
                    fprintf(fp, "%d,%d,%d,no name\n", x, payout.first, payout.second);
                }
            }
        }
    ...
    {
        printf("start saving budgets data\n");
        FILE* fp = fopen(path, "w");
        if (!fp)
        {
            fprintf(stderr, "fopen %s failed\n", path);
    ...
    void backup()
    {
        ...
        backup_name_mapper = name_mapper;
        backup_name_counter = name_counter;
    }
    ...
    void restore()
    {
        ...
        name_mapper = backup_name_mapper;
        name_counter = backup_name_counter;
    }
