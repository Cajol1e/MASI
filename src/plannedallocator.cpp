...
{
    d->buffer = 0;
    d->allocations.resize(4);
    d->counters.resize(4, 0);
...
void* PlannedAllocator::fastMalloc(int memory_type, size_t size)
{
    ...
    {
        NCNN_LOGE("PlannedAllocator::fastMalloc() failed to allocate %d bytes from memory type %d,counter = %d", (int)size, memory_type, d->counters[memory_type]);
        return 0;
    }
...
int PlannedAllocator::get_load_mode()
{
    return d->load_mode;
}
