...
public:
    ...
    int memory_type; // 0 for weight, 1 for blob, 2 for intermediate, 3 for persistent
    int count;
    std::string name = "";
...
{
    return d->memory_profiler->fastMalloc(d->layer_index, d->memory_type, size, d->name);
}
...
void MemoryProfilerInterface::set_attributes(int layer_index,std::string name, int memory_type)
{
    d->layer_index = layer_index;
    d->name = name;
    if (memory_type != -1)
...
void* MemoryProfiler::fastMalloc(int layer_index, int memory_type, size_t size, std::string name)
{
    d->lock.lock();

    void* ptr = ncnn::fastMalloc(size); // actual size = size + NCNN_MALLOC_OVERREAD

    d->events.push_back(MemoryProfilerEvent(layer_index, memory_type, 1, ptr, size + NCNN_MALLOC_OVERREAD, name));
