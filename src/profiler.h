...
public:
    ...
    MemoryProfilerEvent(int _layer_index, int _memory_type, int _event_type, void* _ptr, size_t _size,std::string _name)
        : layer_index(_layer_index), memory_type(_memory_type), event_type(_event_type), ptr(_ptr), size(_size), name(_name)
    {
        time = get_current_time();
    }
...
public:
    ...
    void set_attributes(int layer_index,std::string name, int memory_type = -1);
...
public:
    ...
    void* fastMalloc(int layer_index, int memory_type, size_t size, std::string name);
